...
cubesat = PrimitiveCubesat(
    position=[earth_radius + sat_altitude - relative_distance_to_satellite, relative_distance_to_satellite, 0], #TODO: Define position from Orbit
...
satellite.target_pointing(cubesat.get_position()) #We explicitely say that the satellite has to point to the target
...
scene_manager.render_image(
    ouput_image_path=Path.home().joinpath('exemple_image.png')
)