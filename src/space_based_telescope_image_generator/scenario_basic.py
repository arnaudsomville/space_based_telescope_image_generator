...
sat_altitude = 500.0  # Altitude du satellite en km
relative_distance = 0.05  # Distance relative pour positionner le CubeSat
fov_degree = 10  # Champ de vision en degrés
...
spy_satellite = TrackingSatellite(
    position=[0, 0 ,earth_radius + sat_altitude],
    fov=fov_degree
)
...
print(f'Relative distance to the target : {spy_satellite.compute_relative_distance(cubesat)}')
# Rendu
